...
class Decoder:
	def __init__(self,_filename="",_fileperms="rb",_start=0):
		self.PESectionHeader = PESectionHeader.PESectionHeader()
		self.fields = self.PESectionHeader.header_fields
		self.fmt = "".join([self.PESectionHeader.header_fmt_dict[name] for name in self.fields])
